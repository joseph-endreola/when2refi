// Totality property test for interpretBreakEven.

import { describe, it, expect } from 'vitest';
import fc from 'fast-check';
import type { BreakEvenResult } from '@when2refi/math';
import { interpretBreakEven } from '../interpretBreakEven';
import {
  GOAL_OUTCOME_KINDS,
  dollarArb,
  rateSolverResultArb,
  ...
  it('is total and maps each rate-solver kind per the §4 table', () => {
    fc.assert(
      fc.property(resultArb, ctxArb, (result, ctx) => {
        const outcome = interpretBreakEven(result, ctx);
