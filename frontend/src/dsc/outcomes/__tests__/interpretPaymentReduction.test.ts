// Totality property test for interpretPaymentReduction: every reachable math
// result kind produces a valid GoalOutcome, and the §4 mapping table holds.

import { describe, it, expect } from 'vitest';
import fc from 'fast-check';
import type { PaymentReductionResult } from '@when2refi/math';
import { interpretPaymentReduction } from '../interpretPaymentReduction';
import {
  GOAL_OUTCOME_KINDS,
  dollarArb,
  rateSolverResultArb,
...
const resultArb: fc.Arbitrary<PaymentReductionResult> = fc.record({
  rateSolverResult: rateSolverResultArb,
  display: fc.record({
    closingCost: dollarArb,
    targetPayment: signedDollarArb,
    monthlySavings: dollarArb,
    breakEvenMonths: dollarArb,
  ...
  it('is total and maps each rate-solver kind per the §4 table', () => {
    fc.assert(
      fc.property(resultArb, ctxArb, (result, ctx) => {
        ...
        // Term-reset warning law: additionalMonths is the term delta.
        if (outcome.kind !== 'infeasible') {
